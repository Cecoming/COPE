'''
Non-prompt similarity scoring (NPSS) can be plugged into other methods that do not include prompt scores.
For details, please refer to the supplementary of our paper.
Note: When used in conjunction with COPE, 
      the model output needs to be adjusted to a version that does not include prompt scores.
'''
...
class R1_mAP_eval():
    ...
    def compute(self):  # called after each epoch
        ...
        if self.reranking:
            ...
            dist = re_ranking(qf, gf, k1=50, k2=15, lambda_value=0.3)

        else:
            print('=> Computing DistMat with euclidean_distance')
            dist = euclidean_distance(qf, gf)

        ############################### Non-Prompt Similarity Scoring(NPSS)
        print('=> Compute DistMat with Non-Prompt Score')
